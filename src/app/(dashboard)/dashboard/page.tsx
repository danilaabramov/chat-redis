...
const page = async ({}) => {
    ...
    const friendsWithLastMessage = await Promise.all(
        friends.map(async (friend) => {
            const [lastMessageRaw] = await db.zrange(
    ...
    return (
        <div className='container py-12'>
            <h1 className='font-bold text-5xl mb-8'>Последние чаты</h1>
                        ...
                        <Link href={`/dashboard/chat/${chatHrefConstructor(session.user.id, friend.id)}`}
                            ...
                            <div className='mb-4 flex-shrink-0 sm:mb-0 sm:mr-4'>
                                <div className='relative h-6 w-6'>
                                    <Image referrerPolicy='no-referrer' className='rounded-full'
                                           alt={`${friend.name} profile picture`} src={friend.image} fill/>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <p className='mt-1 max-w-md'>
                                    <span className='text-zinc-400'>
                                        {friend.lastMessage.senderId === session.user.id ? 'Вы: ' : ''}
                                    </span>
